...
export default function ScheduleModal({ editId, preDate, preTime, schedules, contacts, services, onClose }: Props) {
  ...
  // 🔥 CORREÇÃO CRÍTICA: prioriza preDate (data da ocorrência clicada)
  const [date, setDate] = useState(preDate || editing?.date || dateStr(today))
  ...
  // 🔥 ALTERADO: envia a data correta da ocorrência
  async function handleDelete() {
    if (!editId || !confirm('Excluir agendamento?')) return
    setLoading(true)

    console.log('DELETE:', editId, date)

    await deleteSchedule(editId, date)
